...
def define_data_set(T_Ei_conditions, **kwargs):
    ...
    for T, E in T_Ei_conditions:
        data_set = {
            "Runs": list(range(69587, 69788 + 1)),  # not accurate
            ...
            "MdeName": "Ge_{}K_{}meV".format(T, E),  # Options:mde_name string
            "BackgroundMdeName": "",  # Options:None;bg_mde_name string
            "MaskingDataFile": shared_folder
            + "van67625.nxs",  # Options:None;data_file_name
            "NormalizationDataFile": shared_folder
            + "van67625.nxs",  # Options:None;data_file_name
            "SampleLogVariables": {
                "OmegaMotorName": "CCR16Rot",
                "Temperature": T,
            },  # found name by doing hdfview on *_event.nxs and checking under entry->DASlogs
            "UBSetup": {
                "a": 5.66,
                "b": 5.66,
                "c": 5.66,
