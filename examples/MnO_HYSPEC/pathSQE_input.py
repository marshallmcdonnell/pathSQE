...
def define_pathSQE_params(**kwargs):
    ...
    'sample':'Mn O', # separate elements with spaces; don't include stoichiometric subscripts, e.g. for Sr3As2, do 'Sr As'; if desired, some specific isotopes can be specified, e.g. instead of 'Fe Si', could do '56Fe 28Si'
    'space group':'Fm-3m', # don't worry about spaces
    'T and Ei conditions':[(5,25)], # (Temperature, Indicent energy) for each dataset in a list. MDEs must follow consistent naming scheme you specify in define_data.py
    ...
    'E bins':'0,0.2,25', # min, step, max for energy binning
    ...
    # saving and outputs
    'output directory':'../MnO_25meV_5K_unpol_bzFold/', # where to save everything (will be created if doesn't exist)
