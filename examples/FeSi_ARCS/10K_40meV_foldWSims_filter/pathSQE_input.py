...
def define_pathSQE_params(**kwargs):
    ...
    'T and Ei conditions':[(10,40)], # (Temperature, Indicent energy) for each dataset in a list. MDEs must follow consistent naming scheme you specify in define_data.py
    'use seeKpath path':False, # POSCAR required
    'user defined Qpoints':{'path':[ ('Gamma','X'), ('X','M'), ('M','Gamma'), ('Gamma','R'), ('R','X') ], #  e.g. [('Gamma', 'X'), ('X', 'U')], total piece-wise path along which to process 2D inelastic slices, names must match strings in 'point_coords'
                            '1d_points':[], # e.g. ['Gamma', 'X'], all 1d points at which to make 1d S(E) cuts, names must match strings in 'point_coords'
                            'point_coords':{ 'Gamma': [0.0, 0.0, 0.0], 'X': [0.0, 0.5, 0.0], 'M': [0.5, 0.5, 0.0], 'R': [0.5, 0.5, 0.5] }}, # Q point definitions in Q basis associated with MDE's UB matrix
    ...
    'slice filter functions':['M_bragg_tails'], # list of string(s), for automatic slice evaluation and filtering, string(s) must match filter function(s) specified in filter_functions.py
    ...
    'supercell dimensions':[2,2,2], # supercell dimensions used to generate FORCE_CONSTANTS
    'use experimental coverage mask':True, # whether to impose experimental Q,E coverage mask on simulations
    'resolution blurring':(1.4,0.06,'ARCS'), # (E FWHM in meV, Q FWHM in rlu, optional instrument string); if instrument, E FWHM should be elastic line FWHM
