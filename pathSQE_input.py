    ...
    pathSQE_params = {
        ...
        "user defined Qpoints": {
            "path": [
                ...
                ("L", "W"),
                ("W", "X"),
            ],  # e.g. [ ('X', 'U'), ('K', 'Gamma') ], total piece-wise path along which to process 2D inelastic slices, names must match strings in 'point_coords'
            "1d_points": [
                "K",
                "L",
            ],  # e.g. ['Gamma', 'X'], all 1d points at which to make 1d S(E) cuts, names must match strings in 'point_coords'
            "point_coords": {
                "Gamma": [0.0, 0.0, 0.0],
                "X": [0, 1, 0],
                "L": [0.5, 0.5, 0.5],
        ...
        "E bins": "0,0.5,80",  # min, step, max for energy binning
        ...
        "BZ to process": 0.7,  # float or list of lists; if float between [0,1], then BZ with fractional qE coverage above provided threshold are processed (e.g. 0.5); if list of lists, data in each listed BZ will be processed (e.g. [[1,1,1], [2,0,0], [4,2,0]])
        ...
        "mesh": [
            ...
            11,
            11,
        ],  # mesh sampling phonon calculation for Debye-Waller factor
        "use experimental coverage mask": True,  # whether to impose experimental Q,E coverage mask on simulations
        "resolution blurring": (
            0.8,
            0.05,
            "ARCS",
        ),  # (E FWHM in meV, Q FWHM in rlu, optional instrument string for SNS); if instrument, E FWHM should be approx. elastic line FWHM
        # saving and outputs
        "output directory": "../Si_ARCS_80meV_300K/",  # filepath where to save everything (will be created if doesn't exist)
        "save individual slices": True,
        "save reports": True,
        # misc.
        "u_vec": np.array([1, 1, 1]),  # nominal - used for plots
        "v_vec": np.array([1, -1, 0]),  # ^^
