...
def plot_SED_along_path_foldedBZ(foldedSegNames,dsl_fold,pathSQE_params):
    ...
    for i in range(num_segments):
        ...
        SaveMD(SED_ws, Filename=os.path.join(pathSQE_params['output directory'],'slices/fully_folded','{}2{}_folded_SED.nxs'.format(dsl_fold[i]['seg_start_name'],dsl_fold[i]['seg_end_name'])))
        ...
        if k == 0:
            ...
            if pathSQE_params['perform simulations']:
                ...
                if pathSQE_params['save individual slices']:
                    np.save(os.path.join(pathSQE_params['output directory'],'slices/all_individual/',slice_name+'_simSpectrum.npy'), np.vstack((freq_range, spectrum)))
                    np.save(os.path.join(pathSQE_params['output directory'],'slices/all_individual/',slice_name+'_simScatter.npy'), np.vstack((unique_freq, summed_intensities)))
...
def generate_BZ_Report_with_Sims(pathSQE_params, BZ_offset, all_sliceInfo_in_BZ, dsl_fold, folded_results, all_sims_in_BZ):
    ...
    with PdfPages(pdf_filename) as pdf:
        ...
        fig_path_foldedBZ2 = plot_along_path_foldedBZ_sim(sim_folded_data,dsl_fold,pathSQE_params['T and Ei conditions'][0][1],simMin,simMax,'viridis')
