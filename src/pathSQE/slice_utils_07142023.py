...
def make_slice(
    ...
    if is_chi:
        ...
        if not mtd.doesExist(mde_name + "_chi"):
            ApplyDetailedBalanceMD(
                InputWorkspace=mde_name,
                Temperature=str(data_set["SampleLogVariables"]["Temperature"]),
                OutputWorkspace=mde_name + "_chi",
            )
    ...
    if bg_mde_name:
        ...
        if is_chi:
            if not mtd.doesExist(bg_mde_name + "_chi"):
                ApplyDetailedBalanceMD(
                    InputWorkspace=bg_mde_name,
                    Temperature=str(data_set["SampleLogVariables"]["Temperature"]),
                    OutputWorkspace=bg_mde_name + "_chi",
                )
            bg_mde_name += "_chi"

        if (
            mtd[bg_mde_name].getSpecialCoordinateSystem()
    ...
    if SmoothingFWHM:
        ...
        if bg_mde_name:
            ...
            DivideMD(
                LHSWorkspace="_bkg_data",
                RHSWorkspace="_bkg_norm",
                OutputWorkspace="_bkg",
            )

            MinusMD(
                LHSWorkspace=slice_name, RHSWorkspace="_bkg", OutputWorkspace=slice_name
            )
    ...
    if ASCII_slice_folder:
        ...
        SaveMDToAscii(
            mtd[slice_name], filename, IgnoreIntegrated, NumEvNorm=False, Format="%.6e"
        )
    if MD_slice_folder:
...
def make_slices_FR_corrected(
    ...
    SF_F, SF_1 = FlippingRatioCorrectionMD(
        InputWorkspace=mde_SF, FlippingRatio=FR, SampleLogs=var_names
    )
    NSF_F, NSF_1 = FlippingRatioCorrectionMD(
        InputWorkspace=mde_NSF, FlippingRatio=FR, SampleLogs=var_names
    )
...
def subtract_incoherent(ws, q_range=None):
    ...
    params, _ = curve_fit(
        gauss, E[tofit], data_to_use[tofit], [b, a, c, w], np.sqrt(error2_to_use[tofit])
    )
...
def plot_w_parameters(
    ...
    ax[0].text(
        0.01,
        0.99,
        slice_des2str(slice_description, instr=ds["MdeName"]),
        ha="left",
        va="top",
    )
    ax[0].axis("off")
