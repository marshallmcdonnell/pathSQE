...
def polarizer_transmission(ws, **kwargs):
    ...
    ws = ExponentialCorrection(
        ws, C0=1 / 0.585, C1=1 / 10.77, Operation="Multiply"
    )  # was 58.5% *exp(-Ef/12.07)
    ws = ScaleX(ws, Factor=EFixed, Operation="Add")
    return ws


def detector_geometry_correction(ws, psda=0.0):
    """
...
def generate_mde(data_set):
    ...
    if isinstance(runs, dict):
        try:
            ...
            print(
                "The file {} was not found in {}.\n  Try specifying the full path".format(
                    runs["file"], os.getcwd()
                )
            )
            raise FNF
    ...
    polarization_state = data_set.get(
        "PolarizationState"
    )  # Options:None;'SF_Px';'NSF_Px';'SF_Py';'NSF_Py';'SF_Pz';'NSF_Pz'
    flipping_ratio = data_set.get(
        "FlippingRatio"
    )  # Options:None;'14';'6.5+2.8*cos((omega+3.7)*pi/180),omega'
    psda = data_set.get(
        "PolarizingSupermirrorDeflectionAdjustment"
    )  # Options:None;deflection_angle
    Ef_correction_function = data_set.get(
        "EfCorrectionFunction"
    )  # Options: None;'HYSPEC_default_correction';Custom_Ef_Correction_Function_Name
    ...
    for num, run in enumerate(runs):
        ...
        filenames = []
        patterns = [
            data_set["RawDataFolder"] + "*{}.nxs.h5".format(r) for r in run
        ]  #'*{}_event.nxs' for old files
        for p in patterns:
            print(p)
            filenames.extend(glob.glob(p))
        if len(filenames) < 1:
            patterns = [
                data_set["RawDataFolder"] + "*{}_event.nxs".format(r) for r in run
            ]
            for p in patterns:
                filenames.extend(glob.glob(p))
        if len(filenames) < 1:
        ...
        if bad_pulses_threshold:
            data = FilterBadPulses(
                InputWorkspace=data, LowerCutoff=bad_pulses_threshold
            )
        if mask_workspace:
        ...
        if inst_name in ["HYSPEC", "CNCS"]:
            Ei = (
                Ei_supplied
                if Ei_supplied
                else run_obj["EnergyRequest"].getStatistics().mean
            )
        ...
        if Ef_correction_function == "HYSPEC_default_correction":
            ...
            func = locals()[
                Ef_correction_function
            ]  # Ef_correction_function must be loaded in memory
    ...
    if len(runs) > 1:
        MergeMD(
            ",".join(["__{}_{}".format(mde_name, num) for num in range(len(runs))]),
...
def generate_BG_mde(data_set, compress_events_tof):
    ...
    filenames = []
    patterns = [
        bkg_data_folder + "*{}.nxs.h5".format(r) for r in runs
    ]  #'*{}_event.nxs' for old files
    for p in patterns:
        filenames.extend(glob.glob(p))
    data = LoadEventNexus(filenames[0])
    ...
    if len(CheckForSampleLogs(Workspace=data, LogNames="pause")) == 0:
        data = FilterByLogValue(
            InputWorkspace=data, LogName="pause", MinimumValue=-1, MaximumValue=0.5
        )
