...
class ConstantRes(Resolution):
    ...
    def Gauss(self, evalues, qvalues, ecenter, qcenter):
        ...
        Res = (
            ...
            * Nq
            * np.exp(-(q_square) / (2 * self.sigmaq**2))
        )
        return Res
...
class PolynomialRes(Resolution):
    ...
    def Gauss(self, evalues, qvalues, ecenter, qcenter):
        ...
        )  # *Nq*np.exp(-(q_square)/(2*self.sigmaq**2))
        return Res
...
class InstrumentResolution(Resolution):
    ...
    def Gausselastic(self, evalues, ecenter, escale):
        ...
        Res = Ne * np.exp(
            -0.5 * (evalues - ecenter) ** 2 / (self.sigmaconst() ** 2)
        )  # *Nq*np.exp(-0.5*(q_square)/((fwhmq*self.sigmaq)**2))
        # print(np.trapz(Res))
        return Res
    ...
    def GaussQelastic(self, qvalues, qcenter, qscale):
        ...
        Res = np.exp(-(0.5 / qscale) * (q_square) / ((sigma) ** 2)) / np.sum(
            np.exp(-(0.5 / qscale) * (q_square) / ((sigma) ** 2))
        )
        # plt.plot(Res)
    ...
    def sigma(self, ecenter, escale):
        ...
        dE = (
            ...
            * m_neut
            * np.sqrt(
                ((vi**3 / L1) + L2 * vf**3 / (L1 * L3)) ** 2 * fwhm_mon1**2
                + ((vi**3 / L1) + (L1 + L2) * vf**3 / (L1 * L3)) ** 2 * fwhm_mon2**2
                + ((vf**3 / L3) ** 2 * (tsamp / vf) ** 2)
            )
        )  ## BANDER modified 1.5*fwhm*
        dE = dE * joulesTomeV
        return dE
    ...
    def McVinesigma(self, ecenter, escale):
        ...
        mcvinesigma = np.load(
            "/path/to/Interpolated_arcs_res_ARCS-100-1.5-AST_420.0_Ei_50.0.npy"
        )
    ...
    def Gauss(self, evalues, qvalues, ecenter, qcenter, escale, qscale):
        ...
        # q_square=np.dot(delta_q,delta_q)
        Res = Ne * np.exp(
            -((evalues - ecenter) ** 2) / (2 * self.sigma(ecenter, escale) ** 2)
        )  # *Nq*np.exp(-(q_square)/(2*self.sigmaq**2))
    ...
    def GaussQ(self, qvalues, qcenter, qscale):
        q_diff = qvalues - qcenter  ## This is an array of vectors.
        q_square = np.hstack([np.linalg.norm(q) ** 2 for q in q_diff])
        Res = np.exp(-(0.5 / qscale) * (q_square) / ((self.sigmaq) ** 2)) / np.trapz(
            np.exp(-(0.5 / qscale) * (q_square) / ((self.sigmaq) ** 2))
        )
        # print(np.trapz(Res))
        return Res
    ...
    def Voigt(self, evalues, ecenter, escale):
        ...
        Res = (
            ...
            * (gamma)
            * evalues
            / ((evalues**2 - (ecenter) ** 2) ** 2 + (0.5 * (gamma) * evalues) ** 2)
        )
        return Res / np.trapz(Res)
    ...
    def PseudoVoigt(self, evalues, ecenter, escale):
        b = 0.5
        return b * self.GaussMcVine(evalues, ecenter, escale) + (1 - b) * self.Laurentz(
            evalues, ecenter, escale
        )
...
class InstrumentResolutionNoQResolution(Resolution):
    ...
    def sigma(self, ef):
        ...
        dE = m_neut * np.sqrt(
            ((vi**3 / L1) + L2 * vf**3 / (L1 * L3)) ** 2 * fwhm_mon2**2
