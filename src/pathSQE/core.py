...
def find_BZ_with_data(mde_data, pathSQE_params, hkl_range=(-10, 10, -10, 10, -10, 10)):
    ...
    slice_desc = {
        "QDimension0": helper.conv_to_desc_string(
            prim2current[0]
        ),  # make it current2prim columns instead of simple HKL aligned basis
        ...
        for hi in h_idx:
            for ki in k_idx:
                for li in l_idx:
                    ...
                    num_valid_bins += np.sum(
                        np.logical_and(~np.isnan(octant_data), octant_data != 0)
                    )
    ...
    sort_idx = np.lexsort(
        (-bz_norms, -coverage)
    )  # Primary: coverage, Secondary: BZ norm
    BZ_full_array = BZ_full_array[sort_idx]

    # Save to file
    np.savetxt(
        os.path.join(pathSQE_params["output directory"], "BZ_coverage_sorted.txt"),
        np.column_stack([BZ_full_array[:, 0], BZ_full_array[:, 1]]),
        fmt="%s",
        header="BZ_center   Fraction_Coverage",
    )
    ...
    BZ_uv_all = helper.project_to_uv_plane(
        BZ_full_list, pathSQE_params["u_vec"], pathSQE_params["v_vec"]
    )  # All BZs with data
    BZ_uv_threshold = helper.project_to_uv_plane(
        BZ_list, pathSQE_params["u_vec"], pathSQE_params["v_vec"]
    )  # BZs above coverage threshold
    np.save(
        os.path.join(pathSQE_params["output directory"], "BZ_uv_all.npy"), BZ_uv_all
    )
    np.save(
        os.path.join(pathSQE_params["output directory"], "BZ_uv_threshold.npy"),
        BZ_uv_threshold,
    )

    plt.figure(figsize=(6, 6))
    plt.scatter(
        BZ_uv_all[:, 0], BZ_uv_all[:, 1], color="black", s=10, label="BZ with Data"
    )
    plt.scatter(
        BZ_uv_threshold[:, 0],
        BZ_uv_threshold[:, 1],
        color="red",
        s=20,
        label=f"BZ > {min_fraction:.2f} Coverage",
    )
    ...
    BZ_list.sort(
        key=lambda x: (x[1], np.linalg.norm(x[0])), reverse=True
    )  # Sort descending by coverage, secondary by BZ norm
...
def find_qdim_1and2(qdim0, u, v):
    ...
    if np.linalg.norm(qdim1) < 1e-10:  # qdim0 is collinear with z_scattering
        ...
        qdim1 = (
            u - np.dot(u, qdim0) * qdim0
        )  # Start with u and remove its component along qdim0
        ...
        if np.linalg.norm(qdim1) < 1e-10:
            raise ValueError(
                "Failed to construct qdim1. Check input vectors for collinearity issues."
            )
...
def find_qbins(pathSQE_params, qdim0, qdim1, qdim2, pt1, pt2, BZ_offset):
    ...
    CoB_mat = np.linalg.inv(
        np.array([qdim0, qdim1, qdim2]).T
    )  # back to og, edited from Bi version with np.array([qdim0,qdim1,qdim2])
    ...
    qdim0_range = np.array(
        [path_start_transf[0], pathSQE_params["qdim0 step size"], path_end_transf[0]]
    )
    ...
    qdim1_range = np.array(
        [path_start_transf[1] - qdim1_int_range, path_end_transf[1] + qdim1_int_range]
    )

    qdim2_int_range = pathSQE_params["qdim2 integration range"]
    qdim2_range = np.array(
        [path_start_transf[2] - qdim2_int_range, path_end_transf[2] + qdim2_int_range]
    )
...
def choose_dims_and_bins(
    pathSQE_params, point1, point2, u, v, BZ_offset=np.array([0, 0, 0])
):
    diff = point2 - point1
    ...
    qdim0_range, qdim1_range, qdim2_range = find_qbins(
        pathSQE_params, qdim0, qdim1, qdim2, point1, point2, BZ_offset
    )
...
def generate_unique_paths(mtd_spacegroup, pt1, pt2, pathSQE_params):
    ...
    rotations = [
        np.array(
            [
                so.transformHKL((1, 0, 0)),
                so.transformHKL((0, 1, 0)),
                so.transformHKL((0, 0, 1)),
            ]
        )
...
def combine_data_within_bz(data, norm, mtd_data, mtd_norm):
    ...
    data.setErrorSquaredArray(
        data.getErrorSquaredArray() + mtd_data.getErrorSquaredArray()
    )
    return data, norm
...
def fold_BZs(BZ_fold_list, base_BZ, pathSQE_params, user_defined_Qpoints, rerun=False):
    ...
    data_and_norm_dir = os.path.join(
        pathSQE_params["output directory"], "slices/intraBZ_folded/DataAndNorms/"
    )
    ...
    for path_seg in seg_names:
        ...
        if first_data_file is None or first_norm_file is None:
            print(
                f"    Skipping {path_seg}: Missing data or norm files in reference BZ {base_BZ}"
            )
            continue
        ...
        if len(data_files) != len(norm_files):
            print(
                f"    Warning: Mismatched data/norm file count for segment {path_seg}"
            )

        for dfile, nfile in zip(data_files, norm_files):
            data_temp = LoadMD(
                Filename=os.path.join(data_and_norm_dir, dfile), LoadHistory=False
            )
            norm_temp = LoadMD(
                Filename=os.path.join(data_and_norm_dir, nfile), LoadHistory=False
            )
            ...
            data.setErrorSquaredArray(
                data.getErrorSquaredArray() + data_temp.getErrorSquaredArray()
            )
        ...
        if rerun:
            ...
            output_dir = os.path.join(
                pathSQE_params["output directory"], "slices/fully_folded"
            )
        ...
        LoadMD(
            Filename=output_dir + f"/{path_seg}_folded.nxs",
            OutputWorkspace=path_seg,
            LoadHistory=False,
        )
